...
**Test Command (run multiple times quickly):**
```bash
for i in {1..15}; do
  curl -X POST http://localhost:3002/api/transfer \
    -H "Content-Type: application/json" \
...
**Expected Result:** After 10 requests, you'll receive:
```json
{
...
**Answer:** It prevents brute force attacks, reduces server load, and limits the impact of automated attacks.
...
| Rate Limiting | ❌ No limits | ✅ express-rate-limit |
